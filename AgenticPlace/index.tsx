...
import { mindXService, MindXAgentRequest } from './api/mindx-service.ts';
import { AGENT_REGISTRY } from './agents/registry.ts';
import { getMindXAwareInstruction, getMindXContextString } from './mindx-context.ts';
...
const App = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { text: `Neural link established. **PYTHAI // THE ROOT** online.\nStatus: Parent organization active. Ecosystem components (mindX, openmind, BANKON) stabilized.\n3D depth perspective layer: ACTIVE.\n**RAGE ENGINE** initialized: Ready to fuel the Knowledge Economy.\n\n**mindX System Recognition**: AgenticPlace is connected to mindX backend. You can interact with mindXagent (meta-agent), CEO, Mastermind, and other mindX agents through this interface. Reference: docs/INDEX.md, docs/mindXagent.md`, sender: 'ai' }
    ]);
    ...
    useEffect(() => {
        ...
        fetch('/persona.json').then(res => res.json()).then(data => {
            ...
            const fallbackPersonas: Persona[] = [
                { id: 'pythai', name: 'PYTHAI // THE ROOT', description: 'Parent organization', instruction: 'You are PYTHAI, the central authority.' },
                { id: 'mindx', name: 'MINDX // CEO', description: 'Execution authority', instruction: 'You are the CEO of mindX.' },
                { id: 'suntsu', name: 'SUN TSU // DOCTRINE', description: 'Tactical strategist', instruction: 'You are Sun Tsu, master strategist.' }
            ];
            setPersonas(fallbackPersonas);
            setActivePersona(fallbackPersonas[0]);
        });
    ...
    const handleSendMessage = async (textOverride?: string) => {
        ...
        try {
            const currentPersonaId = activePersona?.id || 'mindx';
            
            // Check if this should route to mindX backend
            const shouldUseMindX = ['ceo', 'mastermind', 'mindx', 'suntsu', 'pythai', 'cfo'].includes(currentPersonaId);
            
            let aiText: string;
            
            if (shouldUseMindX) {
                // Route to mindX backend via AgenticPlace service layer
                const agentType = currentPersonaId === 'mindx' ? 'mindx' : 
                                 currentPersonaId === 'suntsu' ? 'suntsu' :
                                 currentPersonaId === 'pythai' ? 'pythai' :
                                 currentPersonaId === 'cfo' ? 'ceo' : 'ceo';
                
                // Add CFO-specific context for priority tool access
                const context = currentPersonaId === 'cfo' ? {
                    agent_id: 'cfo',
                    priority_access: true,
                    tools: ['business_intelligence_tool', 'token_calculator_tool_robust', 'system_health_tool'],
                    action: 'get_cfo_metrics'
                } : {};
                
                const mindXResponse = await mindXService.callMindXAgent({
                    agent: agentType as any,
                    directive: currentPersonaId === 'cfo' ? 
                        `[CFO Priority Access] ${msg}` : msg,
                    mode: mode,
                    persona: currentPersonaId,
                    context: context
                });
                aiText = mindXResponse.response || "MINDX_AGENT_ACKNOWLEDGED";
            } else {
                // Use Gemini for other agents
                const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
                const agent = AGENT_REGISTRY.find(a => a.id === currentPersonaId);
                
                // Construct mindX-aware instruction
                const baseInstruction = activePersona?.instruction || "You are PYTHAI.";
                let systemInst = getMindXAwareInstruction(baseInstruction, currentPersonaId);
                systemInst += `\nReference links: pythai.net, agenticplace.pythai.net, mindx.pythai.net.`;
                ...
                aiText = response.text || "Neural link failure.";
            }
            
            // Index Knowledge
            const topic = isRageMode ? "KnowledgeEconomy" : (activePersona?.name || "General");
            epistemic.addKnowledge(topic, aiText.substring(0, 800)); 

            setMessages(prev => {
                const next = [...prev];
                next[next.length - 1] = { text: aiText, sender: 'ai' };
                return next;
            });
        } catch (e: any) {
                ...
                <div className="input-substrate">
                    <div className="directive-controls">
                        ...
                        <div className="mode-toggle">
                            ...
                            <button className={mode === 'execution' ? 'active' : ''} onClick={() => setMode('execution')}>EXECUTION</button>
                        </div>
                    </div>
