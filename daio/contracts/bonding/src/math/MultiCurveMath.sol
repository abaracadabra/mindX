...
/// @notice Multi-curve math library supporting all bonding curve types.
/// @dev Curve order: POWER, LINEAR, EXPONENTIAL, DECELERATING, TIERED.
library MultiCurveMath {
    ...
    /// @notice Spot price at supply S. Order: POWER, LINEAR, EXPONENTIAL, DECELERATING, TIERED.
    function spotPrice(CurveParams memory params, UD60x18 supply18) internal pure returns (UD60x18) {
        ...
        if (params.curveType == CurveType.POWER) {
            ...
            // P(S) = k * (e^(a*S) - 1)
            UD60x18 aS = params.a.mul(supply18);
            return params.k.mul(exp(aS).sub(ONE));
    ...
    /// @notice Cost to mint delta tokens starting at supply. Order: POWER, LINEAR, EXPONENTIAL, DECELERATING, TIERED.
    function costToMint(CurveParams memory params, UD60x18 supply18, UD60x18 delta18) internal pure returns (UD60x18) {
        ...
        if (params.curveType == CurveType.POWER) {
            ...
            // Integral of k*(e^(a*x)-1) from S to S+Δ = k * ( e^(a*S)*(e^(a*Δ)-1)/a - Δ )
            UD60x18 aS = params.a.mul(supply18);
            UD60x18 aDelta = params.a.mul(delta18);
            UD60x18 term1 = exp(aS).mul(exp(aDelta).sub(ONE)).div(params.a);
            UD60x18 term2 = delta18;
            return params.k.mul(term1.sub(term2));
        } else if (params.curveType == CurveType.DECELERATING) {
    ...
    /// @notice Tokens mintable for given reserve cost. Order: POWER, LINEAR, EXPONENTIAL, DECELERATING, TIERED.
    function mintAmountForCost(CurveParams memory params, UD60x18 supply18, UD60x18 costReserve18) internal pure returns (UD60x18) {
        ...
        if (params.curveType == CurveType.POWER) {
            ...
            return _mintAmountForCostExponential(params, supply18, costReserve18);
        } else if (params.curveType == CurveType.DECELERATING) {
            CurveMath.PowerParams memory pp = CurveMath.PowerParams({k: params.k.mul(ud(2e18)).div(ud(3e18)), p: ud(15e17)});
            return CurveMath.mintAmountForCost(pp, supply18, costReserve18);
        } else if (params.curveType == CurveType.TIERED) {
    ...
    /// @dev Exponential: no closed-form inverse; binary search for Δ given cost.
    function _mintAmountForCostExponential(CurveParams memory params, UD60x18 supply18, UD60x18 costReserve18) private pure returns (UD60x18) {
    ...
    /// @dev Tiered curve cost across phases (linear -> flat -> linear).
    function _costToMintTiered(CurveParams memory params, UD60x18 supply18, UD60x18 delta18) private pure returns (UD60x18) {
        ...
        if (endSupply.gt(params.threshold2)) {
            totalCost = totalCost.add(_tieredPhase3Cost(params, supply18, endSupply));
        }
        return totalCost;
    }

    function _tieredPhase3Cost(CurveParams memory params, UD60x18 supply18, UD60x18 endSupply) private pure returns (UD60x18) {
        ...
        UD60x18 linearPart = deltaSq.div(ud(2e18)).add(offset.mul(phase3Delta).div(ONE));
        return flatCost.add(params.k2.mul(linearPart));
    }

    /// @dev Tiered: iterative solution for Δ given cost.
