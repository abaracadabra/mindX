...
/// Original: DeltaV THRUST (c) DMG EQUITY FINANCE 2022. Rebranded for DAIO.
 ...
 * Rebase protocol with autorewards; locked APY burner + 11.1% afterburner + 1.1% volume burner.
 */
...
/// @title DAIORebaseToken
/// @notice DAIO rebase token: hyper-deflationary autostake rewards. Same mechanics as original; rebranded for DAIO.
contract DAIORebaseToken is ERC20Detailed, Ownable {
    ...
    uint256 private rebaseCounter = 0;
    ...
    uint256 private epoch = rebaseCounter;
    ...
    constructor() ERC20Detailed("DAIO", "REBASE", uint8(DECIMALS)) {
    ...
    function tickRebase() internal returns (bool) {
        deltaT = block.timestamp - inception;
        totalRewards = deltaT / rebaseFrequency;
        if (rebaseCounter <= totalRewards) {
            rebaseCounter = rebaseCounter + 1;
            return true;
        }
        return false;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (tickRebase() && autoRebase && shouldRebase()) {
            _rebase();
