...
**Changelog**: [CHANGELOG.md](./CHANGELOG.md) — the live FACE demo + persona create/clone (2.1), accurate webcam→FAICE capture (2.2), the cloned face speaking in the cloned voice (2.3), **DeltaVerse iNFT minting** — the ERC-7857 payload (2.4) wired to a submittable mint transaction (2.5), the **photoreal renderer** — the FACE lifted from wireframe to a shaded/textured surface (2.6), the **neural renderer** — a gated ONNX seam + a fidelity gate that only stamps hyperrealism when a neural render *measures through it* (2.7), and the **classical enhancement fallback** — weight-free seam-feather · unsharp · tone that improves the affine composite when no neural model is loaded, capped at realism (2.8), **solid hardware handling** — every camera + mic recognised and selectable, facial recognition warmed once with a live tracking indicator (2.9), **WebGL rendering** — the mesh drawn on the GPU with real depth + seam-free texturing, built on the DeltaVerse raw-WebGL substrate with a 2D fallback (2.10), and the **scientific oscilloscope** — YIN frequency measurement, a colour-coded spectrum, edge triggering, zoom, forensic measures (HNR/centroid/rolloff) (2.11), a **spectrogram waterfall** — scrolling time–frequency history on a perceptual inferno colormap (2.12), and **quant-finance accuracy** — a Kalman-stabilised ƒ₀ ± 95% CI, jitter/shimmer, at EVM 18-dp precision, plus a shared sci-fi HUD substrate (with voicey) and interactive waterfall hover/freeze (2.13), **the face as a control surface** — 👁 eye = camera (lens aperture + IPD range finder), 👂 ear = mic, 👄 mouth = output (2.14), and **the full anatomical instrument** — 👃 nose = network diagnostics, nostrils = blockchain scan + RAGE search, 🧠 forehead = analytical/creative tools, 👁 third-eye matrix, ear-speaker pulse, and **morph modes** — **cyborgi** (a polished, reflective, high-detail chrome endoskeleton) · vampire/elf/dragon/pleiadian · age slider · image-upload basis (2.15–2.16), and a **speaking mouth waveform** — the voice drawn as a filled, glowing scope between the lips (2.17). The audio-reactive overlays — the **mouth waveform** and the **ear frequency response** — now render in **every mode, including WebGL 3D**, projected through the camera so they track a turning head (2.18–2.19). Deep dives: **[docs/FACE_CONTROLS.md](./docs/FACE_CONTROLS.md)** · **[docs/OSCILLOSCOPE.md](./docs/OSCILLOSCOPE.md)** · **[docs/WEBGL_RENDER.md](./docs/WEBGL_RENDER.md)** · **[docs/HARDWARE.md](./docs/HARDWARE.md)** · **[docs/FACE_CLONE.md](./docs/FACE_CLONE.md)** · **[docs/NEURAL_RENDERER.md](./docs/NEURAL_RENDERER.md)** · **[docs/PHOTOREAL_RENDER.md](./docs/PHOTOREAL_RENDER.md)** · **[docs/INFT_MINTING.md](./docs/INFT_MINTING.md)**.
**Lineage**: [github.com/Faicey](https://github.com/Faicey) is the original Face-for-AI development environment, kept as a **frozen template**; active code publishes under [Professor-Codephreak/faicey](https://github.com/Professor-Codephreak/faicey). Together with [`voaice`](https://github.com/Professor-Codephreak/voaice) (the voice) and `facerig` (the rig), faicey (the face) composes **[aivatar](https://github.com/Professor-Codephreak/aivatar)** — the definitive `.persona` creation tool: a person's AI model that looks, speaks, rigs and thinks, at an *earned* fidelity tier (basic / professional / scientific → realism · hyperrealism) with consent, custody and signed provenance.
