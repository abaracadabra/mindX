...
- [Algorandscout — reading Algorand through a Blockscout-shaped API](blockchain/algorandscout.md) — the **non-EVM half** of the chain-read surface. [`OpenBDK/algorandscout`](https://github.com/openbdk/algorandscout): a standalone, **BANKON-licensed** service that serves [Algorand](https://algorand.co/) via [algod](https://developer.algorand.org/docs/rest-apis/algod/) + [indexer](https://developer.algorand.org/docs/rest-apis/indexer/) through 12 Blockscout-shaped `/api/v2` routes, closing the Algorand gap named in [blockscout.md §7](blockchain/blockscout.md) — where `mindx.algo` (OVERSEER) and BONA FIDE live. Separate service by necessity: Blockscout re-licensed 2026-04-22 (`LicenseRef-Blockscout`) and its clause 5(b)/5(c) forbid distributing a merged module or keeping its licence, while its own carve-out permits **separable** works; and Algorand has no gas, no nonce, no log topics, no reorgs, and 58-char base32 addresses. Honesty invariants enforced by tests: `gas_used`/`nonce`/`abi` and a round's own hash return **null, never a fabricated value**; clawback transfers are named as clawbacks; uint64 amounts never touch a float. 86 tests against **real mainnet fixtures**. **Capability, not yet wired into mindX.**
