...
> **Implementation note.** In the **current 7-soldier+CEO boardroom** (the live tier today, ahead of the 13-seat war-council deployment described above), each member runs from a paired `.agent` + `.persona` file under [`agents/boardroom/`](../agents/boardroom/). The system prompt for every LLM call (vote, roll-call ack, convene) is composed by [`Boardroom._load_member_card()`](../daio/governance/boardroom.py) from both files. See [agents/boardroom_members.md](agents/boardroom_members.md) for the loading mechanism and the live-verification endpoint `/insight/boardroom/cards`. The PYTHAI 13-seat roster below describes the war-council target; 8 of the 8 active boardroom members are confirmed loaded from files in production.
