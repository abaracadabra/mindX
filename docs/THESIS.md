...
mindX addresses this limitation through [InferenceDiscovery](../llm/inference_discovery.py) — a multi-provider inference system that auto-probes, scores, and selects from available LLM providers ([Ollama](https://ollama.com), [vLLM](../agents/vllm_agent.py), [Mistral](https://mistral.ai), [Gemini](https://ai.google.dev), [Groq](https://groq.com), [OpenAI](https://openai.com), [Anthropic](https://anthropic.com), [Together AI](https://together.ai)). The system is not bound to any single model or provider — it reasons from whatever intelligence is available, treating model selection itself as a cognitive decision logged in the [Gödel audit trail](BOOK_OF_MINDX.md).
