...
- **[Boardroom members — prompt + persona loading](agents/boardroom_members.md)** — the CEO and 7 soldiers each run from a paired `.agent` + `.persona` file in `agents/boardroom/`. `Boardroom._load_member_card()` composes the system prompt from both files (description, behavioural traits, operating beliefs, boardroom role); `_query_soldier()`, `roll_call()`, and `convene_stream()` all inject it. New endpoint `GET /insight/boardroom/cards` shows the exact prompts loaded and flags any member that fell back to the hardcoded dict. **Verified 8/8 loaded from files** in production as of 2026-04-27.
