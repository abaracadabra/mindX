...
- **[HERMES_INTEGRATION.md](HERMES_INTEGRATION.md)** (NEW · 2026-05-13, Day-1 shipped) — first concrete absorption from the Hermes / OpenClaw research stack. `agents/skills/` ships the **SKILL.md** procedural-memory format (Hermes-compatible YAML frontmatter + Markdown body, plus the mindX BDI additions `intention_template` / `preconditions` / `postconditions`) over a `SkillStore` that runs every skill through a **screen-before-persist scanner** with five finding classes (`prompt_injection`, `destructive_command`, `data_exfiltration`, `size`, `missing_required`). Direct architectural answer to the **OpenClaw ClawHub-malware vector** (Koi Security 2026: 341 malicious entries in 2,857 ClawHub skills = 12 % rate, Lakera 2026: prompt-injection vectors in 4,310 audited skills) and the **Hermes ALLOW-ALL-defaults gap** (community audit: 4 Critical + 9 High in default config). Day-1 policy: DENY-ALL on any scanner block; warnings refused unless operator authors `created_by="human"` AND `pinned=true`. Curator-class actors (the deferred MASTERMIND-scheduled cleanup job) can archive — never delete — and cannot touch human-authored or pinned skills. 15 tests pass CPU-only.
- **[Hermes Agent Integration Patterns for mindX](operations/Hermes%20Agent%20Integration%20Patterns%20for%20mindX_%20Self-Improving%20Architecture%20Analysismd)** (NEW research · 2026-05-13) — 494-line companion to the OpenClaw report. Decomposes Hermes v0.13.0 "Tenacity" (released 2026-05-07; 864 commits, 588 PRs, 295 contributors; overtook OpenClaw on OpenRouter daily token volume 224 B vs 186 B as of 2026-05-10 — first market signal that "session-native" architecture is a ceiling). Maps four importable primitives onto mindX without touching model weights: (1) skill-distillation loop, (2) bounded markdown + SQLite-FTS5 + pluggable dialectic memory stack, (3) Kanban heartbeat/zombie/hallucination-gate runtime, (4) DSPy + GEPA evolutionary optimizer. Ranked recommendations table at §10. Cross-attribution with the [OpenClaw research](operations/openclaw_mindx_research.md) below.
- **[openclaw_mindx_research.md](operations/openclaw_mindx_research.md)** (NEW research · 2026-05-12) — 280-line OpenClaw + OpenClaw-RL architectural decomposition for mindX integration. **Five highest-leverage transfers**: SKILL.md format (now shipped), pluggable Context Engine pattern (v2026.3.7+ lossless-claw flagship), **hybrid vector+BM25 (70/30) Active Memory** over SQLite + sqlite-vec, plugin manifest-first validation with capability typing, and **OpenClaw-RL** (slime + SGLang + Megatron, Qwen3.5/3.6-aligned — direct upstream for mindXtrain v3.6). Security history dossier: ClawJacked CVE (Gateway loopback exemption brute-force), Koi Security audit (341 malicious skills), Lakera audit (prompt-injection in 4,310 skills), Anthropic April 2026 detection routing OpenClaw users to higher billing tier (50× cost increase, "do not bind mindX to any single LLM-provider's flat-rate subscription"). Pre-mainnet safety stack: signed skill manifests via BANKON `bankon.eth` subnames, BONAFIDE Tessera on-chain provenance, capability-declared permissions, Unicode steganography detection (zero-width / RTL overrides), static prompt-injection scanning (shipped), Podman sandbox, trust-tier display on AgenticPlace (verified-publisher / community / unscanned).

## 🆕 Recent additions (2026-05-02)

- **[X402.md](X402.md)** (NEW · 2026-05-02) — canonical doc for HTTP 402 micropayments on Algorand. Covers (1) the [x402.org](https://x402.org) spec, (2) Algorand DevRel's just-released [`@x402-avm/*`](https://github.com/algorand-devrel/x402-demo) v^2.6.1 packages (ISC, alpha, TestNet-only, hosted facilitator at `x402.goplausible.xyz`), (3) mindX's pre-standard adoption (EVM-only Base USDC + Tempo MPP via EIP-3009), (4) the **triple-rail convergence** plan (Base + Tempo + Algorand ASA in one `accepts[]` envelope), (5) wire format, (6) operator runbook + four BANKON Vault keys (`algorand_mnemonic`, `algorand_recipient_address`, `algorand_usdc_asa_id`, `x402_avm_facilitator_url`), (7) limitations & roadmap (the deferred `recordAvmReceipt()` extension to `BankonPaymentRouter.sol` for L1 audit parity). New code: [`tools/x402_avm_client.py`](../tools/x402_avm_client.py) (pure Python via `py-algorand-sdk`, mirrors `keeperhub_x402_client.py`), [`AgenticPlace/api/x402_avm.ts`](../AgenticPlace/api/x402_avm.ts) (wraps `@x402-avm/{fetch,hono,avm,core}`). Backend: `_payment_avm` body lift + 402 envelope pass-through + new `/p2p/x402/facilitator-info` discovery route in [`mindx_backend_service/agenticplace_routes.py`](../mindx_backend_service/agenticplace_routes.py). Gated behind `MINDX_X402_AVM_ENABLED=true`.
...
- **[Boardroom × vLLM — true continuous batching](agents/boardroom_vllm.md)** — selector `BOARDROOM_INFERENCE_BACKEND={auto|vllm|ollama}` routes the 8 boardroom members through a self-hosted vLLM server when one is reachable. vLLM continuous batching = all 8 prompts in flight on one model, generation interleaves at the per-token level, all finish ~together (vs sequential per-vote with Ollama). Three deploy paths documented: Modal/RunPod cloud GPU (~$0.50/hr), LAN GPU server, or vLLM CPU on the VPS (slower but free). `auto` mode lets you defer the deploy — when vLLM appears at `BOARDROOM_VLLM_BASE_URL`, the boardroom switches to it on next call without a restart. Health endpoint `/insight/boardroom/health` shows live reachability.
- **[Memory audit + tier-aware retention + the cognitive-ascent loop](MEMORY_AUDIT_2026_04_27.md)** (NEW · 2026-04-27) — the disk filled twice in 24h despite a 5 GB recovery; the audit found the gap was architectural, not bytes. **Three gaps named**: (1) `agent_workspaces/{agent}/process_trace.jsonl` is written by `log_process` and read by nobody — 19.5 GB orphan in one mastermind file; (2) IPFS offload Phase 8 dormant because no Lighthouse keys vaulted; (3) wisdom-tier `*_training.jsonl` files write-only, no consumer. **Six phases shipped this session**: per-tier policy doc ([`memory_tiers.md`](memory_tiers.md)), `GET /insight/memory/audit` (live byte counts), `GET /insight/memory/tiers` (policy + state), tier-aware `memory_agent.prune_stm` (14 d ↔ IPFS eligibility), `agents/storage/agent_workspace_pruner.py` (rotate-archive-delete with disk-headroom guard), `POST /insight/memory/prune` (operator-triggered). **Live recovery freed 19.5 GB**; appendix in the audit doc preserves the incident and the safety guards added (commit `b6d943a5`).
- **[Memory tier policy](memory_tiers.md)** (NEW · 2026-04-27) — single source of truth for "how long does each `/data` path live." Eight tiers: `information_stm` (14 d → archive → IPFS → delete), `knowledge_ltm` / `concept_dreams` / `wisdom_training` (forever — substrates for THOT), `dream_reports` (90 d → archive), `archive` (90 d → IPFS → delete), `thot_anchored` (forever, immutable on chain), `orphan_workspaces` (rotate at 100 MB · archive at 30 d · delete at 90 d). Three hard guarantees: nothing under `ltm/*` ever pruned; no delete until IPFS CID confirmed; default `dry_run=True` everywhere. Live state at `/insight/memory/tiers`.
- **Wisdom-tier consumer — `agents/cognition/wisdom_loader.py`** (NEW · 2026-04-27) — closes the cognitive-ascent loop's ingestion edge. Reads `*_training.jsonl` rows produced by every dream cycle, embeds the assistant message + first 200c of context into pgvector via the existing `memory_pgvector.embed_and_store_doc()` with a `wisdom:{agent}:{ts}:{idx}` doc-name prefix. Idempotent on re-index. Three endpoints: `POST /insight/cognition/wisdom/index?hours=24` (bulk re-index recent files), `GET /insight/cognition/wisdom/search?q=&top_k=3` (semantic retrieval — designed for BDI `perceive()` consumption), `GET /insight/cognition/wisdom/stats` (DISTINCT count). Auto-fires from `machine_dreaming.run_dream_cycle` after `_write_training_data`; first run indexed **91 wisdom docs** on production. Phase 6b — BDI `plan()` prompt prepends "RELEVANT WISDOM" preamble — is the next step that closes the loop end-to-end.
- **[Boardroom self-adaptation — pattern→action recovery registry](agents/boardroom_self_adaptation.md)** — when the roll-call fails, the failure is rarely random: 7/7 soldiers tend to fail for the **same** reason. `Boardroom._diagnose_recovery()` pattern-matches the storm and emits a structured `recovery` block on every rollcall response. Four patterns wired today: `ollama_signin_lapsed` (auto-fires `/insight/boardroom/cloud_signin`, severity high), `cold_load_storm` (3 remediation hints, no auto-action), `empty_ack_drift` (diagnoses prompt/num_ctx truncation), `persona_files_missing` (names the affected seats). Surfaced as a banner above `/feedback.html#sec-board` rollcall + as CEO → SELF dialogue messages on `/boardroom`. Adding a 5th pattern is one entry — matcher predicate + recovery dict. Foundation for mindX adapting to its own errors.
- **[`/dreams.html`](https://mindx.pythai.net/dreams.html) — machine.dreaming with diagnostics** — dedicated page for the 7-phase consolidation cycle. Hero strip shows total dreams · STM bytes freed · LTM bytes grown · median compression ratio · training rows · last cycle age. Operator-triggered "Accelerate dream now" button POSTs `/insight/dreams/run` for an immediate cycle. Recent-cycles table with byte-delta colour coding (green = STM freed, cyan = LTM grown, amber = compression ratio). Click any cycle → `/insight/dreams/diff/{filename}` drill-down with per-agent cards: STM input excerpts (raw memory files), LTM consolidated JSON, and the first training-jsonl rows. **Each dream now writes a parallel `*_training.jsonl`** in `data/memory/ltm/<agent>/` — finetuning-ready chat-completion examples (`{messages: [{role: system|user|assistant, ...}]}`) suitable for OpenAI/Anthropic API or local LoRA. Substrate for promoting verified concepts to machine.wisdom and on-chain THOT mints.
