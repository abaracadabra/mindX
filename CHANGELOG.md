...
> ETHGlobal Open Agents submission cut. **158 tests passing** across 7 suites; **9 live consoles** at `https://mindx.pythai.net/`; CI green on every job.
...
- 20 pytest cases (`tests/bankon_vault/test_shadow_overlord.py` + `test_cabinet.py`) — including the headline `test_sign_as_agent_returns_valid_sig_no_pk_leak` that proves the cryptographic invariant
...
- `.github/workflows/test.yml` — GitHub Actions CI running all 7 test suites in parallel; 158/158 tests green in clean environment
...
| Suite | Tests | Result |
|---|---|---|
| Cabinet pytest (Python) | 20 | ✅ 3.17s |
