...
- `/insight/identity/algorand` - **Independent verification of mindX's own OVERSEER identity.** Reads the configured `MINDX_ALGO_ADDRESS` back from Algorand through [Algorandscout](https://github.com/openbdk/algorandscout) (loopback `:8100`, systemd `algorandscout.service`) and compares it against what mindX assumes. Grades findings: `account_rekeyed` and `account_absent` are **critical**, below-min-balance a warning, multisig informational. **Why it exists:** the OVERSEER check verifies an Ed25519 signature against the key embedded in the address, and is structurally blind to **rekeying** — after a rekey, on-chain authority is the account's `auth-addr`, so a signature check keeps accepting the key that is no longer in charge. **No fallback by design:** if Algorandscout is unreachable the verdict is `unavailable`, never a pass. The payload carries its own `not_verified` list (the chain shows authority, not custody). `?h=true` renders plain text. Re-checked hourly by `run_identity_monitor` (`MINDX_IDENTITY_CHECK_*`), which alerts only on **transitions** — a silent baseline, one alert per change, no re-alerting
...
- `agents/catalogue/events.py` — Pydantic `CatalogueEvent` with typed `kind`s including `memory.{write,consolidate,dream,offload,anchor}`, `godel.choice`, `board.{session,vote}`, `tool.{invoke,result}`, `skill.{invoke,result}`, `alignment.score`, `identity.verified` (chain-read verdict on mindX's own OVERSEER account), `links.checked` (outbound link sweep), `improvement.{proposed,executed}`, `agent.interact`, `publication.{attempted,published,coalesced}`, `marketing.*`, `admin.*`
