...
# Admin-auth hook — the HOST app sets this (standalone bankoneth wires the
# admin-tier session in backend/app.py; mindX wires security_middleware). It is
# resolved PER-REQUEST so the host can configure it after this module imports.
# Until set, every gated route 503s (never silently allows unauth access).
require_admin_access = None  # type: ignore  # set by the host; Callable[[Request], Awaitable]
try:  # pragma: no cover — only when embedded in mindX
    from mindx_backend_service.security_middleware import require_admin_access as _mindx_admin
    require_admin_access = _mindx_admin
except Exception:
    pass
...
async def _admin_gate(request: Request):
    """Per-request admin gate — dispatches to the host-configured hook."""
    hook = require_admin_access
    if hook is None:
