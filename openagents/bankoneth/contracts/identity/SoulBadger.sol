...
import "./interfaces/IAgenticPlace.sol";

/**
 * @title SoulBadger - Soulbound Token Implementation
 * @dev Non-transferable NFTs for permanent agent credentials
 * @notice Implements ERC-5484 (Soulbound Token) standard for DAIO
 * @notice Soulbound tokens are permanently bound to their owner and cannot be transferred
 * @dev Extends AgenticPlace for credential verification in marketplace
 */
contract SoulBadger is ERC721, AccessControl {
    bytes32 public constant BADGE_ISSUER_ROLE = keccak256("BADGE_ISSUER_ROLE");
    IAgenticPlace public agenticPlace; // Foundational marketplace reference
     ...
     */
    function setAgenticPlace(address _agenticPlace) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /**
     * @notice Verify user has credential badge (for marketplace verification)
    ...
    /**
     * @dev Prevent transfers to enforce soulbound behavior (ERC-5484 compliant)
     * @notice Only allows minting (from == address(0)) and burning (to == address(0))
     */
    function _update(
        address to,
        uint256 tokenId,
        ...
        // ERC-5484: Soulbound tokens can only be minted or burned, never transferred
        require(
