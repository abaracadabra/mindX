...
| **15** | [immutable contract naming](https://docs.ens.domains/web/naming-contracts) | reverse | `setReverseName(rr, name)` admin method on `BankonSubnameRegistrar`, `BankonEthRegistrar`, `BankonDomainHosting`, `BankonOffchainRegistrar`. Wired by [`SetPrimaryNames.s.sol`](../script/SetPrimaryNames.s.sol). Forward duality via [`SetForwardNames.s.sol`](../script/SetForwardNames.s.sol) + round-trip via [`VerifyContractNames.s.sol`](../script/VerifyContractNames.s.sol). Full audit: [`CONTRACT_NAMING_AUDIT.md`](CONTRACT_NAMING_AUDIT.md) |
| **19** | [multichain reverse (draft)](https://ensips.ethereum.org/ensips/19) | reverse | TS helpers shipped — [`reverseNamespace`](../packages/core/src/contract-naming.ts) + `l2ReverseRegistrarFor` + `L2_REVERSE_REGISTRARS` map. ENSIP-3 + ENSIP-19 namespace derivation tested for Optimism/Base/Arbitrum at [`test/ContractNaming.t.sol:Ensip19NamespaceTest`](../test/ContractNaming.t.sol). On-chain L2 deploy itself deferred. |
