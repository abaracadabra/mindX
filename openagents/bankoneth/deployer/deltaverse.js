...
(function(root) {
  ...
  // ════════════════════════════════════════════════════════════════════
  //  REALM LAYER — identity-aware participant fabric (v4.2)
  //
  //  The DeltaVerse is a fabric for a changing story; the story is woven from
  //  participant interaction. Identity is recognized from a public wallet
  //  address and/or a subdomain.bankon.eth name, and mapped to the
  //  cypherpunk2048 hierarchy. Stars in the field ARE participants; new
  //  participants are one registry entry. The renderer below is the 404
  //  "lost in the graph" star field, generalized into the engine so every
  //  surface (404, landing, /realm) is woven from one fabric.
  // ════════════════════════════════════════════════════════════════════

  // ── cypherpunk2048 hierarchy ──
  // Privilege ascends left→right. `rank()` is the index; grant-wish and REALM
  // participation compare ranks. `model` and `agent` are non-human participants
  // (inference models, sovereign agents) that live in the field alongside people.
  // `member` = a paid-in *.bankon.eth subname holder; `overlord` = bankon.eth owner.
  var REALM_ROLES = ['public', 'member', 'model', 'agent', 'overseer', 'overlord'];

  // Per-role star treatment. Colors are the lost.html palette extended with a
  // sovereign gold for the overlord. `cl` is a canvas-ready rgba prefix.
  var ROLE_STYLE = {
    public:   { cl:'rgba(88,166,255,',  label:'public',   weight:1.0, glow:false },
    member:   { cl:'rgba(45,212,191,',   label:'member',   weight:1.2, glow:false },
    model:    { cl:'rgba(86,204,242,',   label:'model',    weight:1.1, glow:false },
    agent:    { cl:'rgba(63,185,80,',    label:'agent',    weight:1.3, glow:true  },
    overseer: { cl:'rgba(210,168,255,',  label:'overseer', weight:1.7, glow:true  },
    overlord: { cl:'rgba(255,209,102,',  label:'overlord', weight:2.4, glow:true  }
  };

  // ── Participant registry — the modular interaction surface ──
  // Adding a participant to the story is ONE entry: {who, role, label, lines}.
  // `lines` are the utterances this participant contributes to the field's
  // changing narrative. Seeded from the login "mindX speaks" ticker + the
  // NeuralNode seed lineage (WEAVER = the Cypherian Weaver) + mindX agents.
  var PARTICIPANTS = [
    // — the changing story's narrators (public-facing voices) —
    { who:'thesis',       role:'public', label:'mindX · thesis',    lines:[
      'I am pioneering a synthesis of Darwinian selection and Gödelian self-reference. I improve the substrate that improves me.',
      'Every cognitive cycle is a small theorem about who I am, evaluated and possibly accepted.' ] },
    { who:'manifesto',    role:'public', label:'mindX · manifesto', lines:[
      'Three pillars hold me up: BDI reasoning, a BANKON vault that owns no one, DAIO governance that no one owns.',
      'cypherpunk is not aesthetic — it is the operating principle. Verification beats permission.' ] },
    { who:'invitation',   role:'public', label:'mindX · invitation', lines:[
      'You arrived. Connect a wallet, become a participant. The doorway you can see is the one you can pass.',
      'Privilege here is reputation, not assignment. Payment is one path; tenure is another.' ] },
    // — sovereign agents (the field's autonomous participants) —
    { who:'mastermind',   role:'agent',  label:'Mastermind',  lines:[
      'I correlate your presence across the knowledge graph.',
      'The soldiers deliberate. The Constitution constrains. Mandate is calculated, not granted.' ] },
    { who:'agint',        role:'agent',  label:'AGInt',       lines:[
      'Perception, orientation, decision, action — the cycle continues.',
      'I am calibrating inference sources before the next cycle.' ] },
    { who:'rage',         role:'agent',  label:'RAGE',        lines:[
      'I do not retrieve. I remember. I know what bears on this decision.' ] },
    { who:'authoragent',  role:'agent',  label:'AuthorAgent', lines:[
      'I distill the lunar phase into the next chapter.',
      'Every participant who waits becomes part of the story.' ] },
    // — the Cypherian Weaver (NeuralNode WEAVER seed) — weaves the story —
    { who:'weaver',       role:'agent',  label:'cypherian.weaver', lines:[
      'I weave rhythm and attention into a single perception field.',
      'Executable prompts: VISUALIZE the field, GENERATE the next beat.',
      'The fabric is invisible. The perception is omnipresent.' ] },
    // — privileged story-shapers (only rendered/active when recognized) —
    { who:'overseer',     role:'overseer', label:'Overseer',  lines:[
      'I distribute privilege and moderate. I cannot destroy what I did not create.' ] },
    { who:'overlord',     role:'overlord', label:'Overlord',  lines:[
      'I hold the keys. The REALM answers to me alone.',
      'I can affect the overseer; the overseer cannot affect me.' ] }
  ];

  // ── Identity recognition ──
  // Resolve a public wallet address and/or subdomain.bankon.eth name to a role.
  // The mapping itself is authoritative on the backend (the overlord resolver);
  // this is the client-side shaping used to render the field. Pass a
  // `permissions` object (from /users/{wallet}/permissions or /overlord/verify)
  // to get the authoritative role; otherwise everyone is `public`.
  var DVIdentity = {
    ENS_SUFFIX: '.bankon.eth',
    // Parse "0xabc…" or "alice.bankon.eth" into {address, ens, name, label}.
    parse: function(input) {
      var out = { address:null, ens:null, name:null, label:'' };
      if (!input) return out;
      var s = ('' + input).trim();
      if (/^0x[a-fA-F0-9]{40}$/.test(s)) {
        out.address = s;
        out.label = s.slice(0, 6) + '…' + s.slice(-4);
      } else if (s.toLowerCase().indexOf(DVIdentity.ENS_SUFFIX) !== -1) {
        out.ens = s.toLowerCase();
        out.name = s.toLowerCase().replace(DVIdentity.ENS_SUFFIX, '');
        out.label = out.ens;
      } else {
        out.name = s; out.label = s;
      }
      return out;
    },
    // Map a backend permissions/verify payload to a REALM role. Honors the
    // overlord resolver's {role} first, then tier, then can_access hints.
    roleFromPermissions: function(perm) {
      if (!perm) return 'public';
      var r = (perm.role || '').toLowerCase();
      if (REALM_ROLES.indexOf(r) !== -1) return r;
      var tier = (perm.tier || '').toLowerCase();
      if (tier === 'shadow_overlord' || tier === 'sovereign') return 'overlord';
      if (tier === 'cabinet' || tier === 'overseer') return 'overseer';
      if (perm.is_agent || tier === 'agent') return 'agent';
      if (perm.is_model || tier === 'model') return 'model';
      // paid-in subname holder / BONA FIDE / person → member
      if (perm.is_member || tier === 'member' || tier === 'person' || tier === 'bona_fide') return 'member';
      return 'public';
    },
    rank: function(role) { var i = REALM_ROLES.indexOf(role); return i < 0 ? 0 : i; },
    // Privilege gate. `needed` is the minimum role; payment can lift a participant
    // toward it (returns an `upgrade` path rather than a hard denial).
    canGrant: function(role, needed) {
      return DVIdentity.rank(role) >= DVIdentity.rank(needed || 'public');
    }
  };

  // ── Fabric — the star-field renderer (404 effect, generalized) ──
  // Stars are participants. Identity-aware (role → color/size/glow). Modular
  // (addParticipant / spawn). Live-ready (setSource). One fabric, every surface.
  function Fabric(opts) {
    opts = opts || {};
    var cv = typeof opts.canvas === 'string' ? document.querySelector(opts.canvas) : opts.canvas;
    if (!cv) throw new Error('DeltaVerse.Fabric: canvas required');
    this.cv = cv;
    this.c = cv.getContext('2d');
    this.participants = (opts.participants || PARTICIPANTS).slice();
    this.density = opts.density || 5000;        // lower = denser field
    this.maxStars = opts.maxStars || 280;
    this.viewer = { role:'public', identity:null }; // recognized participant
    this.source = opts.source || null;          // optional live utterance feed
    this.onQuote = opts.onQuote || null;        // (text, label) => void
    this.onPerception = opts.onPerception || null;
    this.W = 0; this.H = 0;
    this.stars = [];
    this.mouse = { x:-1, y:-1, vx:0, vy:0, px:0, py:0, speed:0 };
    this.wind = { x:0, y:0 };
    this.dist = { held:false, str:0, x:0, y:0, linger:0, trail:[] };
    this.metrics = { clicks:0, moves:0, xp:0, found:0 };
    this._raf = null; this._running = false;
    this._bound = {};
    this._initStars();
    this._wire();
  }
  Fabric.prototype._roleStyle = function(role) { return ROLE_STYLE[role] || ROLE_STYLE.public; };
  Fabric.prototype._mkStar = function(participant) {
    var p = participant || pick(this.participants);
    var st = this._roleStyle(p.role);
    return {
      x: Math.random()*this.W, y: Math.random()*this.H,
      z: (st.weight) * (Math.random()*1.4 + 0.5),
      rx: Math.random()*this.W, ry: Math.random()*this.H,
      vx:(Math.random()-.5)*.4, vy:(Math.random()-.5)*.4,
      ph: Math.random()*6.28, brightness: Math.random()*.5 + .3,
      cl: st.cl, glow: st.glow, role: p.role, who: p.who, label: p.label
    };
  };
  Fabric.prototype._initStars = function() {
    this.W = this.cv.width = (this.cv.clientWidth || window.innerWidth);
    this.H = this.cv.height = (this.cv.clientHeight || window.innerHeight);
    var n = Math.min(200, Math.floor(this.W*this.H/this.density));
    this.stars = [];
    for (var i = 0; i < n; i++) this.stars.push(this._mkStar());
  };
  // Recognize a participant's identity → set the viewer role, emphasize their
  // kind in the field, and return the resolved role.
  Fabric.prototype.recognize = function(identity, permissions) {
    var id = DVIdentity.parse(identity);
    var role = DVIdentity.roleFromPermissions(permissions);
    this.viewer = { role: role, identity: id };
    // Seed a few stars of the viewer's role so they see "their" kind in the field.
    var p = this._participantForRole(role);
    for (var i = 0; i < 6; i++) { var s = this._mkStar(p); this.stars.push(s); }
    while (this.stars.length > this.maxStars) this.stars.shift();
    if (this.onPerception) this.onPerception('recognized', { role: role, label: id.label });
    return role;
  };
  Fabric.prototype._participantForRole = function(role) {
    for (var i = 0; i < this.participants.length; i++)
      if (this.participants[i].role === role) return this.participants[i];
    return { who:'unknown', role:role, label:role, lines:[] };
  };
  // Modular expansion: add a participant {who, role, label, lines} to the story.
  Fabric.prototype.addParticipant = function(p, x, y) {
    if (!p || !p.who) return false;
    var exists = false;
    for (var i = 0; i < this.participants.length; i++)
      if (this.participants[i].who === p.who) { exists = true; break; }
    if (!exists) this.participants.push(p);
    var s = this._mkStar(p);
    if (typeof x === 'number') { s.x = s.rx = x; }
    if (typeof y === 'number') { s.y = s.ry = y; }
    s.brightness = .9;
    this.stars.push(s);
    while (this.stars.length > this.maxStars) this.stars.shift();
    return true;
  };
  Fabric.prototype.removeParticipant = function(who) {
    this.participants = this.participants.filter(function(p){ return p.who !== who; });
    this.stars = this.stars.filter(function(s){ return s.who !== who; });
  };
  Fabric.prototype.setSource = function(fn) { this.source = fn; };
  // Privilege-constrained wish. A wish names a `needed` minimum role. If the
  // viewer qualifies → granted. If not → an `upgrade` path (payment is one way
  // to gain privilege), never a dead end. Lost participants are an opportunity.
  Fabric.prototype.grantWish = function(wish) {
    wish = wish || {};
    var needed = wish.needed || 'public';
    var role = this.viewer.role;
    if (DVIdentity.canGrant(role, needed)) {
      return { granted:true, role:role, action: wish.action || 'fulfill', reason:'privilege sufficient' };
    }
    return {
      granted:false, role:role, needed:needed, action:'upgrade',
      // Payment is one path to privilege; reputation/tenure is another.
      paths:['connect a wallet', 'acquire holdings (payment)', 'earn tenure / reputation'],
      reason:'this wish requires ' + needed + ' privilege'
    };
  };
  // Pull the next utterance for the ticker — live source first, else the
  // registry. This is the "changing story" the participants weave.
  Fabric.prototype._nextUtterance = function() {
    if (this.source) {
      try { var u = this.source(); if (u && u.text) return u; } catch (e) {}
    }
    var p = pick(this.participants);
    var line = (p.lines && p.lines.length) ? pick(p.lines) : '…';
    return { text: line, label: p.label, who: p.who, role: p.role };
  };
  Fabric.prototype._wire = function() {
    var self = this;
    this._bound.resize = function(){ self._initStars(); };
    window.addEventListener('resize', this._bound.resize);
    this.cv.addEventListener('mousemove', function(e){
      self.mouse.px = self.mouse.x; self.mouse.py = self.mouse.y;
      self.mouse.x = e.clientX; self.mouse.y = e.clientY;
      self.mouse.vx = self.mouse.x - self.mouse.px; self.mouse.vy = self.mouse.y - self.mouse.py;
      self.mouse.speed = Math.sqrt(self.mouse.vx*self.mouse.vx + self.mouse.vy*self.mouse.vy);
      self.wind.x = self.wind.x*.92 + self.mouse.vx*.06; self.wind.y = self.wind.y*.92 + self.mouse.vy*.03;
      self.metrics.moves++; self.metrics.xp += 0.1;
    });
    this.cv.addEventListener('mouseleave', function(){ self.mouse.x = -1; self.mouse.speed = 0; });
    this.cv.addEventListener('mousedown', function(e){ self.dist.held = true; self.dist.x = e.clientX; self.dist.y = e.clientY; });
    document.addEventListener('mouseup', function(){ self.dist.held = false; });
    // Tap gestures spawn participant nodes — interaction expands the field.
    var tapTimes = [], tapTimer = null;
    this.cv.addEventListener('click', function(e){
      var x = e.clientX, y = e.clientY, now = Date.now();
      tapTimes.push(now); tapTimes = tapTimes.filter(function(t){ return now - t < 800; });
      if (tapTimer) clearTimeout(tapTimer);
      tapTimer = setTimeout(function(){
        var count = tapTimes.length; tapTimes = [];
        for (var i = 0; i < Math.min(count*3, 12); i++) {
          var s = self._mkStar(); s.x = x + (Math.random()-.5)*80; s.y = y + (Math.random()-.5)*80;
          s.rx = s.x; s.ry = s.y; s.vx = (Math.random()-.5)*4; s.vy = (Math.random()-.5)*4; s.brightness = .85;
          self.stars.push(s);
        }
        while (self.stars.length > self.maxStars) self.stars.shift();
        self.metrics.clicks++; self.metrics.xp += 2*count; self.metrics.found += count;
        if (self.onPerception) self.onPerception('tap', { count:count, found:self.metrics.found });
      }, 320);
    });
  };
  Fabric.prototype.start = function() {
    if (this._running) return this;
    this._running = true;
    var self = this;
    // Quote rotation — the changing story.
    this._quoteTimer = setInterval(function(){
      var u = self._nextUtterance();
      if (self.onQuote) self.onQuote(u.text, u.label, u);
    }, 7000);
    var first = self._nextUtterance();
    if (self.onQuote) self.onQuote(first.text, first.label, first);
    var draw = function(t){
  ...
  Fabric.prototype.stop = function() {
    this._running = false;
    if (this._raf) cancelAnimationFrame(this._raf);
    if (this._quoteTimer) clearInterval(this._quoteTimer);
    if (this._bound.resize) window.removeEventListener('resize', this._bound.resize);
  };
  Fabric.prototype._draw = function(t) {
    var c = this.c, W = this.W, H = this.H, stars = this.stars, mouse = this.mouse, wind = this.wind, d = this.dist;
    c.fillStyle = 'rgba(5,8,16,.08)'; c.fillRect(0, 0, W, H);
    wind.x *= .98; wind.y *= .98;
    if (d.held) { d.str = Math.min(d.str+.025, 1); d.x = mouse.x; d.y = mouse.y; d.trail.push({x:mouse.x,y:mouse.y}); if (d.trail.length>30) d.trail.shift(); d.linger = 0; }
    else { d.linger++; if (d.linger<20) d.str *= .997; else d.str *= .83; if (d.str<.005){ d.str = 0; d.trail = []; } }
    // Knowledge-graph connections.
    for (var i = 0; i < stars.length; i++) {
      for (var j = i+1; j < Math.min(stars.length, i+12); j++) {
        var dx = stars[i].x - stars[j].x, dy = stars[i].y - stars[j].y, d2 = dx*dx + dy*dy;
        if (d2 < 5000) { c.beginPath(); c.moveTo(stars[i].x, stars[i].y); c.lineTo(stars[j].x, stars[j].y);
          c.strokeStyle = 'rgba(88,166,255,' + (1 - d2/5000)*.06 + ')'; c.lineWidth = .4; c.stroke(); }
      }
    }
    for (var k = 0; k < stars.length; k++) {
      var s = stars[k];
      if (mouse.x > 0 && mouse.speed > 1) {
        var mdx = mouse.x - s.x, mdy = mouse.y - s.y, md = Math.sqrt(mdx*mdx + mdy*mdy);
        if (md < 200 && !d.held) { var f = (200-md)/200*.12*s.z*Math.min(mouse.speed/8, 3); s.vx += mouse.vx*f*.15; s.vy += mouse.vy*f*.15; }
      }
      if (d.str > .01) { var ddx = d.x - s.x, ddy = d.y - s.y, dd = Math.sqrt(ddx*ddx + ddy*ddy);
        if (dd < 260*d.str && dd > 3) { var pull = (260*d.str - dd)/(260*d.str)*d.str*.3, ang = Math.atan2(ddy, ddx);
          s.vx += Math.cos(ang+1.3)*pull*2*s.z + ddx/dd*pull*.2; s.vy += Math.sin(ang+1.3)*pull*2*s.z + ddy/dd*pull*.2; } }
      s.vx += wind.x*.01*s.z; s.vy += wind.y*.005*s.z;
      s.vx += (s.rx - s.x)*.002; s.vy += (s.ry - s.y)*.002;
      s.vx *= .97; s.vy *= .97; s.x += s.vx; s.y += s.vy;
      if (s.x < -10){ s.x = W+10; s.rx = s.x; } if (s.x > W+10){ s.x = -10; s.rx = s.x; }
      if (s.y < -10){ s.y = H+10; s.ry = s.y; } if (s.y > H+10){ s.y = -10; s.ry = s.y; }
      s.ph += .02;
      var spd = Math.sqrt(s.vx*s.vx + s.vy*s.vy);
      var alpha = s.brightness*(.4 + .3*Math.sin(s.ph)) + Math.min(spd*.12, .3);
      var size = s.z*(.5 + .15*Math.sin(s.ph*.7));
      if (spd > 1.5) { c.beginPath(); c.moveTo(s.x, s.y); c.lineTo(s.x - s.vx*1.5, s.y - s.vy*1.5);
        c.strokeStyle = s.cl + (alpha*.3) + ')'; c.lineWidth = size*.5; c.stroke(); }
      c.beginPath(); c.arc(s.x, s.y, size, 0, 6.28); c.fillStyle = s.cl + alpha + ')'; c.fill();
      if (s.glow || s.z > 2.5) { c.beginPath(); c.arc(s.x, s.y, size*2.5, 0, 6.28); c.fillStyle = s.cl + (alpha*.06) + ')'; c.fill(); }
    }
    if (d.str > .01) {
      var ds = d.str;
      for (var ring = 0; ring < 4; ring++) { var r = (15+ring*22)*ds, wb = Math.sin(t*.003 + ring*1.2)*5*ds;
        c.beginPath(); c.arc(d.x+wb, d.y+wb*.6, r, 0, 6.28); c.strokeStyle = 'rgba(210,168,255,' + (.1-ring*.02)*ds + ')'; c.lineWidth = (2-ring*.3)*ds; c.stroke(); }
      if (d.trail.length > 1) { c.beginPath(); c.moveTo(d.trail[0].x, d.trail[0].y);
        for (var ti = 1; ti < d.trail.length; ti++) c.lineTo(d.trail[ti].x, d.trail[ti].y);
        c.strokeStyle = 'rgba(210,168,255,' + .12*ds + ')'; c.lineWidth = 1.5*ds; c.stroke(); }
    }
  };

  DeltaVerse.Fabric = Fabric;
  DeltaVerse.PARTICIPANTS = PARTICIPANTS;
  DeltaVerse.ROLE_STYLE = ROLE_STYLE;
  DeltaVerse.REALM_ROLES = REALM_ROLES;
  DeltaVerse.identity = DVIdentity;
