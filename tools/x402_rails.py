...
EIP / ERC references (definitive — https://eips.ethereum.org/EIPS).
The credential this service issues is, by the books:

* **ERC-3009 — Transfer With Authorization.** The signed
  ``transferWithAuthorization`` envelope; transfer of fungible assets via a
  signed authorization with a random single-use ``nonce`` and a
  ``validAfter``/``validBefore`` window. https://eips.ethereum.org/EIPS/eip-3009
* **EIP-712 — Typed structured data hashing and signing.** How the
  ``TransferWithAuthorization`` struct is hashed and signed.
  https://eips.ethereum.org/EIPS/eip-712
* **EIP-155 — Simple replay attack protection.** The ``chainId`` in the EIP-712
  domain (and the ``eip155:8453`` network id). https://eips.ethereum.org/EIPS/eip-155
* **ERC-20 — Token Standard.** The asset moved (USDC).
  https://eips.ethereum.org/EIPS/eip-20
* **ERC-191 — Signed Data Standard** / **ERC-1271 — Standard Signature
  Validation Method for Contracts.** The verification envelope and the
  contract-wallet (multisig payer) path. https://eips.ethereum.org/EIPS/eip-191
  · https://eips.ethereum.org/EIPS/eip-1271

Full reference table and cypherpunk2048-conformance notes:
``docs/cypherpunk2048/x402_rails.md`` and ``docs/cypherpunk2048/EIP_REFERENCES.md``.
"""
...
from tools.cmc_client import X402_BASE_URL, decode_payment_challenge
from tools.x402_signer import (
    USDC_MINOR_UNITS,
    X402SignerError,
...
class X402RailsService:
    ...
    def issue_from_challenge(
        ...
        try:
            ...
            raise X402RailsError(
                f"signing failed on rail {rail_name}: {exc}"
            ) from exc
        if not payload_b64:
    ...
    def _credential_from_payload(
        ...
        try:
            ...
            raise X402RailsError(
                "rail returned an undecodable payment payload"
            ) from exc
    ...
    def _unwrap(response: httpx.Response) -> dict[str, Any]:
        if response.status_code != 200:
            raise X402RailsError(
                f"settled request failed (http {response.status_code})"
            )
        body = response.json()
